...
- 需要判断平台差异时，再读对应 `.agents/_core/stacks/<stack>.md`

## 规则

- 正式应用用 `vup app add <app-name>`。
- 共享功能包用 `vup package add <package-name>`。
- 示例工程用 `vup example add <example-name>`。
- 不手工复制 `apps/`、`packages/`、`examples/` 下的模板目录来替代 vup 命令。
- 命令可能是交互式时，优先让用户执行并回传输出；只有用户明确同意，AI 才走本地拷贝降级方案。

## 完成后

- 更新 `.agents/project.md` 的 Apps / Packages / Examples / Commands。
