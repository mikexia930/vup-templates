    ...
    <div class="mx-auto w-[min(1120px,calc(100%_-_48px))] max-[680px]:w-[calc(100%_-_32px)]">
      ...
      <div class="border-neutral-3 overflow-hidden rounded-xl border bg-white">
        ...
        <div class="min-h-[220px] p-[28px]">
          <div v-if="activeTab === 'i18n'" class="grid gap-[16px]">
            <div
            ...
            <h3 class="text-xl font-extrabold">{{ t('demo.runtime.i18n.title') }}</h3>
          ...
          <div v-else class="grid gap-[16px]">
            <button
              class="bg-primary-5 hover:bg-primary-6 inline-flex min-h-[42px] w-fit cursor-pointer items-center justify-center rounded-md border border-transparent px-[18px] text-sm font-bold text-white transition disabled:cursor-not-allowed disabled:opacity-70"
              type="button"
            ...
            <div v-if="taskResponse" class="grid gap-[12px] overflow-x-auto">
              ...
              <table class="w-full border-collapse text-sm">
                <thead>
                  <tr>
                    <th
                      class="border-neutral-3 bg-neutral-1 text-neutral-6 border-b px-[14px] py-[11px] text-left text-xs font-bold"
                    >
                      ID
                    </th>
                    <th
                      class="border-neutral-3 bg-neutral-1 text-neutral-6 border-b px-[14px] py-[11px] text-left text-xs font-bold"
                    >
                      {{ t('demo.runtime.http.task') }}
                    </th>
                    <th
                      class="border-neutral-3 bg-neutral-1 text-neutral-6 border-b px-[14px] py-[11px] text-left text-xs font-bold"
                    >
                      {{ t('demo.runtime.http.status') }}
                    </th>
                    <th
                      class="border-neutral-3 bg-neutral-1 text-neutral-6 border-b px-[14px] py-[11px] text-left text-xs font-bold"
                    >
                      {{ t('demo.runtime.http.owner') }}
                    </th>
                  </tr>
                </thead>
                <tbody>
                  <tr v-for="task in taskResponse.items" :key="task.id">
                    <td class="border-neutral-3 border-b px-[14px] py-[11px] text-left">
                      {{ task.id }}
                    </td>
                    <td class="border-neutral-3 border-b px-[14px] py-[11px] text-left">
                      {{ task.name }}
