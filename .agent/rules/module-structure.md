...
## ⚠️ 设计原则：高内聚低耦合 + DRY（强制执行）

> 本节是架构层面的硬性约束，与「核心原则」同等优先级。**违反任一条必须停下修正，不得继续。**

### 高内聚

- **一个模块 / composable / 组件只做一件事**，职责单一、边界清晰
- 模块内部的 views、components、stores、api 围绕同一业务域聚合，不混入无关功能
- 如果一个模块开始承担两个不同业务域的职责 → 必须拆分为两个模块

### 低耦合

- 模块间**只通过 Public API（`index.ts`）通信**，不依赖对方的内部实现细节
- 模块间共享数据优先通过 store 或事件，不要直接 import 对方的内部状态
- 修改一个模块的内部实现时，**不应该需要同时修改其他模块**——如果需要，说明耦合过紧，必须重构

### DRY：禁止重复代码（零容忍）

> **相同逻辑写两遍就是 bug。**
> AI 在编码前必须先搜索已有代码，确认没有可复用的实现再动手写。

- **相同逻辑出现 2 次 → 必须立即提取**为函数 / composable / 组件
- 提取位置规则：
  - 仅当前模块使用 → 模块内 `composables/` 或 `components/`
  - 跨模块使用 → `src/common/`
- **禁止复制粘贴后「微调」**——应通过参数化 / 插槽 / 泛型 / 配置实现差异化
- 编码前 AI 必须执行搜索：检查 `src/common/`、当前模块、其他模块的 `index.ts`
  导出，确认是否已有可复用能力
- **抽象必须可被 AI 发现**：提取的公共能力必须有注释说明用途（详见
  `commenting.md`），让 AI 在后续任务中能搜索到并复用，而不是因为不知道已有实现而重复编写
